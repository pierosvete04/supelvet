...
const regex = /function verDetalle\(id\)\{[\s\S]*?\}function cerrarDetalle\(\)\{/m;
const match = content.match(regex);
if(match) {
    content = content.replace(regex, jsCode + '\nfunction cerrarDetalle(){');
    fs.writeFileSync('index_1_corregido.html', content, 'utf8');
    console.log('verDetalle successfully updated!');
} else {
    console.log('Could not find verDetalle in index_1_corregido.html.');
}
